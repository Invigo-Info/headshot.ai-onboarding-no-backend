...
export async function POST(req: Request) {
 ...
 if(!userImageUrl) {
  ...
  try {
    ...
    const result = (await replicate.run("recraft-ai/recraft-crisp-upscale", { input })) as ReplicateImageOutput;
    ...
    const errorMessage = error instanceof Error ? error.message : "An unknown error occurred while generating the image upscaler";
    console.error(error, errorMessage);

    return new Response(
      JSON.stringify({ error: "An unknown error occurred while generating the image upscaler" }),
      { status: 500 }
    );
  }
}
