...
export async function POST(req: Request) {
  ...
  try {
   ...
   if(!userImageUrl) {
    ...
    // Prepare API input for image upscaling
    const input = {
      image: userImageUrl,
      scale: 2,
      face_enhance: true
    ...
    // Use replicate upscaler for image enhancement
    const output = (await replicate.run("alexgenovese/upscaler:4f7eb3da655b5182e559d50a0437440f242992d47e5e20bd82829a79dee61ff3", { input })) as UpscalerOutput;
    ...
    const errorMessage = error instanceof Error ? error.message : "An unknown error occurred while unblurring the image";
    console.error(error, errorMessage);

    return new Response(
      JSON.stringify({ error: "An unknown error occurred while unblurring the image" }),
      { status: 500 }
    );
  }
}
