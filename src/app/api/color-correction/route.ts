 ...
 if(!userImageUrl) {
  ...
  try {
    ...
    const input = {
      ...
      white_balance_percentile: 90
    };
    
    
    const result = (await replicate.run("fofr/color-matcher", { input })) as ReplicateImageOutput;
    ...
    const errorMessage = error instanceof Error ? error.message : "An unknown error occurred while generating the color correction";
    console.error(error, errorMessage);

    return new Response(
      JSON.stringify({ error: "An unknown error occurred while generating the color correction" }),
      { status: 500 }
    );
  }
}
