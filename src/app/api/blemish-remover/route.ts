...
export async function POST(req: Request) {
 ...
 if(!userImageUrl) {
  ...
  try {
    ...
    // Prepare API input for blemish removal using nano-banana
    const input = {
      prompt: "Remove all blemishes, acne, scars, dark spots, and imperfections from the face. Make the skin look smooth and natural without any blemishes or marks. Maintain the original facial features and expression. Do not change facial structure or permanent features (moles, freckles) unless they’re explicitly listed to remove. Maintain original lighting, color, and expression. Subtle, realistic retouch only.",
      image_input: [userImageUrl],
      output_format: "png"
    };

    // Use replicate nano-banana for blemish removal
    ...
    const errorMessage = error instanceof Error ? error.message : "An unknown error occurred while removing blemishes";
    console.error(error, errorMessage);

    return new Response(
      JSON.stringify({ error: "An unknown error occurred while removing blemishes" }),
      { status: 500 }
    );
  }
}
