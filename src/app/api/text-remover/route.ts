...
export async function POST(req: Request) {
 ...
 if(!userImageUrl) {
  ...
  try {
    ...
    const result = (await replicate.run("flux-kontext-apps/text-removal", { input })) as ReplicateImageOutput;
    ...
    const errorMessage = error instanceof Error ? error.message : "An unknown error occurred while generating the text remover";
    console.error(error, errorMessage);

    return new Response(
      JSON.stringify({ error: "An unknown error occurred while generating the text remover" }),
      { status: 500 }
    );
  }
}
